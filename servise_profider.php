...
function hsp_get_sa_cities() {
    return array(
        ...
        'حائل', 'أبها', 'خميس مشيط', 'نجران', 'جازان',
        'ينبع', 'الجبيل', 'الأحساء', 'القطيف', 'حفر الباطن',
        'عرعر', 'سكاكا', 'الباحة', 'بيشة', 'الخرج',
        'عنيزة', 'الدوادمي', 'المجمعة', 'الزلفي', 'شقراء',
        'الأفلاج', 'وادي الدواسر', 'رفحاء', 'طريف', 'تربة',
        'الليث', 'القنفذة', 'رابغ', 'المذنب', 'الرس',
        'البكيرية', 'رأس تنورة', 'صفوى', 'بقيق', 'النعيرية',
        'القريات',
    );
}
...
function render_hiraaj_pro_metabox($post) {
    ...
    ?>
    <style>
        .hiraaj-box { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; padding: 10px; }
        .hiraaj-field label { display: block; font-weight: bold; margin-bottom: 5px; color: #2271b1; }
        .hiraaj-field input, .hiraaj-field select { width: 100%; padding: 8px; border-radius: 4px; border: 1px solid #ccc; background: #fff; }
        .hiraaj-full { grid-column: span 2; background: #f0f6fb; padding: 15px; border-radius: 8px; border-right: 4px solid #2271b1; }
    </style>
    <div class="hiraaj-box">
        <div class="hiraaj-field hiraaj-full">
            <label>نوع الخدمة:</label>
            <select name="p_role" required>
                <option value="transporter" <?php selected($role, 'transporter'); ?> <?php if(empty($role)) echo 'selected'; ?>>🚛 ناقل (نقل مركبات)</option>
...
add_action('save_post', function($post_id) {
    if (get_post_type($post_id) !== 'service_provider') return;
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
    if (!isset($_POST['hsp_nonce']) || !wp_verify_nonce($_POST['hsp_nonce'], 'hsp_save_meta')) return;
...
function hiraaj_render_service_providers() {
    ...
    if (!is_user_logged_in()) {
        ?>
        <div class="hiraaj-guest-prompt" style="margin: 40px 0; padding: 35px; background: #fff; border: 2px solid #eee; border-radius: 20px; text-align: center; direction: rtl; box-shadow: 0 10px 30px rgba(0,0,0,0.05);">
            <div style="font-size: 45px; margin-bottom: 15px;">👋</div>
            <h3 style="margin: 0 0 10px 0; color: #2c3e50; font-weight: 800;">انضم إلينا لرؤية خدمات النقل والفحص!</h3>
            <p style="color: #7f8c8d; margin-bottom: 25px; font-size: 15px;">سجل دخولك الآن لتظهر لك قائمة السائقين والمعاينين المتوفرين في مدينتك لهذا المنتج.</p>
            <div style="display: flex; gap: 15px; justify-content: center;">
                <a href="<?php echo esc_url(wc_get_page_permalink( 'myaccount' )); ?>" style="background: #0073aa; color: #fff !important; padding: 12px 30px; border-radius: 30px; text-decoration: none; font-weight: bold; font-size: 14px;">تسجيل الدخول</a>
                <a href="<?php echo esc_url(wc_get_page_permalink( 'myaccount' )); ?>?action=register" style="background: #f8f9fa; color: #333 !important; padding: 12px 30px; border-radius: 30px; text-decoration: none; font-weight: bold; border: 1px solid #ddd; font-size: 14px;">إنشاء حساب جديد</a>
            </div>
        </div>
        <?php
        return ob_get_clean();
    ...
    if (empty($user_address)) {
        ?>
        <div class="hiraaj-location-prompt" style="margin: 40px 0; padding: 35px; background: #fff; border: 2px dashed #0073aa; border-radius: 20px; text-align: center; direction: rtl;">
            <div style="font-size: 45px; margin-bottom: 15px;">📍</div>
            <h3 style="margin: 0 0 10px 0; color: #2c3e50; font-weight: 800;">حدد موقعك لرؤية مقدمي الخدمة</h3>
            <p style="color: #7f8c8d; margin-bottom: 25px; font-size: 15px;">يرجى تحديد موقعك على الخريطة في صفحتك الشخصية لنظهر لك المعاينين والناقلين في منطقتك.</p>
            <a href="<?php echo esc_url(wc_get_page_permalink( 'myaccount' )); ?>/edit-account/" style="display: inline-block; background: #0073aa; color: #fff !important; padding: 14px 35px; border-radius: 35px; text-decoration: none; font-weight: bold; box-shadow: 0 5px 15px rgba(0,115,170,0.3);">
    ...
    if (empty($matched_providers)) {
        ?>
        <div class="hiraaj-no-providers" style="margin: 40px 0; padding: 25px; background: #f8f9fa; border-radius: 15px; text-align: center; direction: rtl; border: 1px solid #eee; color: #666;">
    ...
    ?>
    <style>
        .hiraaj-wrapper { position: relative; margin: 40px 0; padding: 20px; background: #fdfdfd; border-radius: 15px; border: 1px solid #eee; direction: rtl; }
        .hiraaj-title { font-size: 18px; font-weight: bold; margin-bottom: 20px; display: flex; align-items: center; gap: 10px; color: #2c3e50; }
        .hiraaj-slider { display: flex; overflow-x: auto; gap: 15px; padding-bottom: 15px; scroll-behavior: smooth; -webkit-overflow-scrolling: touch; scrollbar-width: none; }
        .hiraaj-slider::-webkit-scrollbar { display: none; }
        .hiraaj-card { min-width: 260px; background: #fff; border-radius: 12px; border: 1px solid #eee; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.03); transition: 0.3s; }
        .hiraaj-card:hover { transform: translateY(-5px); box-shadow: 0 8px 20px rgba(0,0,0,0.08); }
        .card-img { height: 130px; background-size: cover; background-position: center; }
        .card-content { padding: 15px; }
        .badge { font-size: 10px; padding: 3px 8px; border-radius: 5px; background: #f8f9fa; color: #666; border: 1px solid #eee; }
        .price-tag { color: #27ae60; font-weight: bold; font-size: 15px; margin: 12px 0; }
        .btn-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
        .btn-s-card { text-align: center; padding: 10px; border-radius: 8px; text-decoration: none !important; font-size: 13px; font-weight: bold; color: #fff !important; }
        .b-call { background: #2ecc71; }
        .b-wa { background: #25D366; }
        .filter-nav { display: flex; gap: 10px; margin-bottom: 15px; }
        .f-btn { cursor: pointer; border: 1px solid #ddd; background: #fff; padding: 6px 15px; border-radius: 20px; font-size: 13px; transition: 0.2s; }
        .f-btn.active { background: #2271b1; color: #fff; border-color: #2271b1; }
    </style>

    <div class="hiraaj-wrapper">
        <div class="hiraaj-title">🚚 خدمات متاحة في منطقتك (<?php echo esc_html(trim(str_replace(['امارة منطقة', 'منطقة'], '', $user_address))); ?>)</div>
            ...
            ?>
                <div class="hiraaj-card" data-role="<?php echo esc_attr($role); ?>">
                    ...
                    <div class="card-content">
                        ...
                        <p style="font-size: 12px; color: #7f8c8d; margin: 0; height: 18px; overflow: hidden;"><?php echo esc_html($desc); ?></p>
                        ...
                        <div class="btn-grid">
                            ...
                            <a href="https://wa.me/966<?php echo ltrim($phone, '0'); ?>" target="_blank" class="btn-s-card b-wa">واتساب</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
...
add_action('manage_service_provider_posts_custom_column', function($column, $post_id) {
    if ($column === 'p_city') echo esc_html(get_post_meta($post_id, '_p_city', true));
    if ($column === 'p_role') echo get_post_meta($post_id, '_p_role', true) === 'transporter' ? 'ناقل' : 'معاين';
    if ($column === 'p_phone') echo esc_html(get_post_meta($post_id, '_c_phone', true));
}, 10, 2);